...
import {closePanel,closeAllPanels} from '../ui/modals.js';
import {renderVersions,saveDoc} from '../ui/sidebar.js';

export function openVersions(){closeAllPanels();state.panelType='versions';$('versionPanel').classList.add('open');if(state.page)renderVersions()}
...
export function deleteVer(vid){if(!confirm('버전 삭제?'))return;state.page.versions=state.page.versions.filter(function(v){return v.id!=vid});saveDB();savePage(state.page);renderVersions();toast('삭제됨')}
